...
";
    ...
    //    [TestMethod]
    //    public void RunConsoleAppWithTotalsNewTest()
    //    {
    //        //Arrange
    //        if (SamplesPath != null && ConsoleExePath != null)
    //        {
    //            string arguments = $"-d {SamplesPath} -t";
    //            Process? process = StartApplication(arguments);
    //            string? actual = null;
    //            string expected = @"Framework,FrameworkFamily,Count,Status
    //.NET 5.0,.NET,2,deprecated
    //.NET 6.0,.NET,4,supported
    ...
    //            if (process != null)
    //            {
    //                Task<string?> outputTask = WaitForResponse(process);
    //                outputTask.Wait();
    //                actual = outputTask.Result;
    //            }

    //            //Asset
    //            //actual = TextHelper.CleanTimingFromResult(actual);
